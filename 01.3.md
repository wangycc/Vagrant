...
* `vagrant box list`
	...
	显示当前已经添加的box列表
		...
		$ vagrant box list
		CentOS7.1  (virtualbox)

* `vagrant box remove <name>`

	删除相应的box

		$ vagrant box remove CentOS7.1 virtualbox
		Removing box 'CentOS7.1' with provider 'virtualbox'...
...
* `vagrant package [options] [name]`
	...
	打包命令，可以把当前的运行的虚拟机环境进行打包
		...
		[default] Compressing package to: /Users/daoke/vagrant/package.box
...
* `vagrant ssh-config`
	...
	输出用于ssh连接的一些信息
		...
		Host default
		  ...
		  IdentityFile "/Users/daoke/.vagrant.d/insecure_private_key"
		  IdentitiesOnly yes
		  LogLevel FATAL
...
现在我们来建立多台VM跑起來，並且让他们之间能够相通信，假设一台是应用服务器、一台是DB服务器，那么这个结构在Vagrant中非常简单，其实和单台的配置差不多，你只需要通过`config.vm.define`来定义不同的角色就可以了，现在我们打开配置文件进行如下设置：
	...
	Vagrant.configure("2") do |config|
	  config.vm.define :web do |web|
	    ...
	    web.vm.hostname = "web"
	    web.vm.network :private_network, ip: "11.11.11.2"
	  end
...
是不是很酷！现在接下来我们再来验证一下虚拟机之间的通信，让我们先使用ssh登录web虚拟机，然后在web虚拟机上使用ssh登录db虚拟机(默认密码是`vagrant`)：
	...
	Warning: Permanently added '11.11.1.3' (RSA) to the list of known hosts.
